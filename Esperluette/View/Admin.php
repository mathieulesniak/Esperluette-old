...
class Admin extends \Fwk\Page
{
    ...
    private function renderNavigation()
    {
        ...
        foreach ($this->sections as $sectionId => $sectionData) {
            ...
            $output .= '<li' . $class . '>' . "\n";
            $output .= '   <a href="' . Helper::url($sectionData['url']) . '"><i class="' . $sectionData['icon'] . '"></i> ' . Helper::i18n($sectionData['name'])  . '</a>' . "\n";
            if (isset($this->subSections[$sectionId])) {
                $output .= '<ul class="gn-submenu">'."\n";
                foreach ($this->subSections[$sectionId] as $currentSubSection) {
                    $output .= '<li><a href="' . Helper::url($currentSubSection['url']) . '">' . Helper::i18n($currentSubSection['name']) . '</a></li>'."\n";
                }
                $output .= '</ul>'."\n";
            }
            $output .= '</li>' . "\n";
